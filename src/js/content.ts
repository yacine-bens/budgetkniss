...
const applyBudget = async ({ budget, isHideOtherItems, isHideNoPriceItems, color } = DEFAULT_PREFS) => {
    ...
    if (priceItems.length) {
        priceItems.forEach(item => {
            ...
            const priceElement = document.evaluate(priceElementXpathExp, item, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue as HTMLDivElement;
...
const getItemsContainer = async () => {
    ...
    return itemsContainerXpathResult.snapshotItem(0) as HTMLDivElement;
}
...
const waitForXPath = async (xpathExpression: string, node: unknown, observedElement: HTMLElement): Promise<XPathResult> => {
    return new Promise((resolve) => {
        if (document.evaluate(xpathExpression, node as Node, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength) {
            return resolve(document.evaluate(xpathExpression, node as Node, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null));
        }
